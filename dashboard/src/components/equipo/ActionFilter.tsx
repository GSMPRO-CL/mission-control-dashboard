...
export function ActionFilter({ actionList, onChange }: ActionFilterProps) {
        ...
        <span className="font-medium">
          {selectedActions.size === 0 
            ...
            : selectedActions.size === 1
              ? Array.from(selectedActions)[0]
              : `${selectedActions.size} acciones`}
        </span>
      </button>

      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-xl overflow-hidden z-50">
          <div className="p-2 border-b border-slate-800 flex justify-between items-center">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Filtrar por Acción</span>
            {selectedActions.size > 0 && (
              <button 
          ...
          <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
            ...
            {actionList.length === 0 && (
              <div className="p-3 text-sm text-slate-500 text-center">No hay acciones disponibles</div>
            )}
          </div>
        </div>
      )}
